...
import {
  Upload, Button, message, notification
} from 'antd';
...
class AvatarView extends PureComponent {
  ...
  handleChange = (info) => {
    ...
    if (info.file.status === 'done') {
      this.setState({
        currentAvatar: `${process.env.REACT_APP_STATIC_SITE}${qiniuKey}`,
      });

      onChangeAvatar(
        `${process.env.REACT_APP_STATIC_SITE}${qiniuKey}`
      );
