        ...
        >
          <template v-for="(item, index) in routes">
            <el-submenu :index="index + ''" v-if="!item.meta.leaf && item.children" :key="index">
              <template slot="title">
                <i :class="item.meta.icon"  class="iconfont mar" />
                <span class="title">{{item.name}}</span>
              </template>
              <el-menu-item v-for="child in item.children" :index="child.path" :key="child.path">
                <i :class="child.meta.icon"  class="iconfont mar" />
                <span class="text">
                  {{child.name}}
                </span>
              </el-menu-item>
          ...
          <el-menu-item index="">
            <a href="https://analytics.google.com" target="_blank">
              <i class="iconfont mar icon-count" />
  ...
  computed: {
    ...
    routes () {
      return this.$router.options.routes.map((r) => {
        if (r.children) {
          const children = r.children.filter(c => !c.meta.leaf)
          return { ...r, children }
        } else {
          return r
        }
